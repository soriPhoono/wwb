...
<template>
  <div class="relative" v-if="user">
    ...
    >
      <!-- Avatar circle -->
      <span class="flex items-center justify-center w-7 h-7 rounded-lg bg-blue-500 text-white text-sm font-extrabold shrink-0">
        {{ initials(user.email) }}
      </span>
      <span class="text-sm font-semibold max-w-[120px] truncate hidden sm:block">{{ user.email }}</span>
      <!-- Chevron -->
      <svg
    ...
    <Transition name="dropdown">
      ...
      >
        ...
        <div class="px-4 py-3 border-b border-slate-100">
          <p class="text-xs text-slate-400 font-semibold uppercase tracking-wider m-0">Signed in as</p>
          <p class="text-sm font-bold text-slate-800 truncate m-0 mt-0.5">{{ user.email }}</p>
        </div>

        <!-- Menu items -->
        <div class="py-1">
          <button
            class="w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer bg-transparent border-none"
          >
